...
const Post = ({ data }) => {
  ...
  const deletePostMutation = useMutation({
    mutationFn: async () => {
      const res = await axios.delete(
        `http://graduation.amiralsayed.me/api/posts/${data._id}`,
        {
  ...
  const likePostMutation = useMutation({
    mutationFn: async () => {
      const res = await axios.post(
        `http://graduation.amiralsayed.me/api/posts/${data._id}/reactions`,
  ...
  const sharePostMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await axios({
        method,
        url: `http://graduation.amiralsayed.me/api/posts/${data._id}/shares`,
        headers: { Authorization: `Bearer ${token}` },
      });
      return res.data;
    },
  ...
  const handleUpdateCaption = async () => {
    ...
    try {
      const response = await axios.put(
        `http://graduation.amiralsayed.me/api/posts/${data._id}/caption`,
        { newCaption },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const savePostMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await axios.post(
        `http://graduation.amiralsayed.me/api/posts/${data._id}/saves`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      <div className="Post">
        ...
        <div>
          ...
          <div className="post-detail">
            ...
            <img
              src={`http://graduation.amiralsayed.me${data.media[0].url}`}
              alt=""
            />
          </div>
          ...
          <div className="postReact">
            <div>
              ...
              />
              <span>{data.impressionsCount.total + data.shareCount}</span>
            </div>
